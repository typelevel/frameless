...
trait AggregateFunctions {
  ...
  /**
   * Aggregate function: returns the number of items in a group for which the selected column is
   * not null.
   *
   * apache/spark
   */
  ...
  def stddevPop[A, T](column: TypedColumn[T, A])(
      implicit ev: CatalystCast[A, Double]): TypedAggregate[T, Option[Double]] = {
   ...
   * value it sees when ignoreNulls is set to true. If all values are null, then null is
   * returned.
   *
   * apache/spark
   */
   ...
   *
   * @note
   *   In Spark corr always returns Double
  ...
  def corr[A, B, T](column1: TypedColumn[T, A], column2: TypedColumn[T, B])(
      ...
      i1: CatalystCast[B, Double]): TypedAggregate[T, Option[Double]] = {
    new TypedAggregate[T, Option[Double]](
   ...
   *
   * @note
   *   In Spark covar_pop always returns Double
   ...
   *
   * @note
   *   In Spark covar_samp always returns Double
   ...
   *
   * @note
   *   In Spark kurtosis always returns Double
   ...
   *
   * @note
   *   In Spark skewness always returns Double
