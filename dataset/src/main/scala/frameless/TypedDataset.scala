 ...
 */
class TypedDataset[T] protected[frameless] (val dataset: Dataset[T])(
  ...
  def agg[A](ca: TypedAggregate[T, A]): TypedDataset[A] = {
    ...
    TypedEncoder[A].catalystRepr match {
      case StructType(_) =>
        ...
        val df = tuple1.dataset.selectExpr("_1.*").as[A](TypedExpressionEncoder[A])
  ...
  object aggMany extends ProductArgs {
    def applyProduct[U <: HList, Out0 <: HList, Out](columns: U)(
        implicit i0: AggregateTypes.Aux[T, U, Out0],
        i1: ToTraversable.Aux[U, List, UntypedExpression[T]],
        i2: Tupler.Aux[Out0, Out],
        i3: TypedEncoder[Out]): TypedDataset[Out] = {
      ...
      val selected =
        dataset.toDF().agg(cols.head, cols.tail: _*).as[Out](TypedExpressionEncoder[Out])
   ...
   * }}}
   */
  def project[U](implicit projector: SmartProject[T, U]): TypedDataset[U] =
    projector.apply(this)
   ...
   * apache/spark
   */
  def union[U: TypedEncoder](other: TypedDataset[U])(
      implicit projector: SmartProject[U, T]): TypedDataset[T] =
  ...
  def apply[A](column: Witness.Lt[Symbol])(
      implicit i0: TypedColumn.Exists[T, column.T, A],
      i1: TypedEncoder[A]): TypedColumn[T, A] = col(column)
  ...
  object colMany extends SingletonProductArgs {
    def applyProduct[U <: HList, Out](columns: U)(
        implicit i0: TypedColumn.ExistsMany[T, U, Out],
        i1: TypedEncoder[Out],
   ...
   * apache/spark
   */
  def show[F[_]](numRows: Int = 20, truncate: Boolean = true)(
      implicit F: SparkDelay[F]): F[Unit] =
  ...
  def filter(column: TypedColumn[T, Boolean]): TypedDataset[T] = {
    val filtered = dataset.toDF().filter(column.untyped).as[T](TypedExpressionEncoder[T])
  ...
  object cubeMany extends ProductArgs {
    def applyProduct[TK <: HList, K <: HList, KT](groupedBy: TK)(
        implicit i0: ColumnTypes.Aux[T, TK, K],
        i1: Tupler.Aux[K, KT],
        i2: ToTraversable.Aux[TK, List, UntypedExpression[T]]): CubeManyOps[T, TK, K, KT] =
  ...
  object groupByMany extends ProductArgs {
    def applyProduct[TK <: HList, K <: HList, KT](groupedBy: TK)(
        implicit i0: ColumnTypes.Aux[T, TK, K],
        i1: Tupler.Aux[K, KT],
        i2: ToTraversable.Aux[TK, List, UntypedExpression[T]]): GroupedByManyOps[T, TK, K, KT] =
  ...
  object rollupMany extends ProductArgs {
    def applyProduct[TK <: HList, K <: HList, KT](groupedBy: TK)(
        implicit i0: ColumnTypes.Aux[T, TK, K],
        i1: Tupler.Aux[K, KT],
        i2: ToTraversable.Aux[TK, List, UntypedExpression[T]]): RollupManyOps[T, TK, K, KT] =
   ...
   * returning a `Tuple2` for each pair where condition evaluates to true.
   */
  def joinLeftSemi[U](other: TypedDataset[U])(
   ...
   * returning a `Tuple2` for each pair where condition evaluates to true.
   */
  def joinLeftAnti[U](other: TypedDataset[U])(
      condition: TypedColumn[T with U, Boolean]): TypedDataset[T] =
    new TypedDataset(
      self
        .dataset
  ...
  def makeUDF[A: TypedEncoder, R: TypedEncoder](
      f: A => R): TypedColumn[T, A] => TypedColumn[T, R] = functions.udf(f)
  ...
  def makeUDF[A1: TypedEncoder, A2: TypedEncoder, R: TypedEncoder](
      f: (A1, A2) => R): (TypedColumn[T, A1], TypedColumn[T, A2]) => TypedColumn[T, R] =
    functions.udf(f)
   ...
   */
  def makeUDF[A1: TypedEncoder, A2: TypedEncoder, A3: TypedEncoder, R: TypedEncoder](
      f: (A1, A2, A3) => R)
      : (TypedColumn[T, A1], TypedColumn[T, A2], TypedColumn[T, A3]) => TypedColumn[T, R] =
    functions.udf(f)
  ...
  def makeUDF[
      ...
      A3: TypedEncoder,
      A4: TypedEncoder,
      R: TypedEncoder](f: (A1, A2, A3, A4) => R): (
      TypedColumn[T, A1],
      TypedColumn[T, A2],
      TypedColumn[T, A3],
      TypedColumn[T, A4]) => TypedColumn[T, R] = functions.udf(f)
  ...
  def makeUDF[
      ...
      A4: TypedEncoder,
      A5: TypedEncoder,
      R: TypedEncoder](f: (A1, A2, A3, A4, A5) => R): (
      TypedColumn[T, A1],
      TypedColumn[T, A2],
      TypedColumn[T, A3],
      TypedColumn[T, A4],
      TypedColumn[T, A5]) => TypedColumn[T, R] = functions.udf(f)
  ...
  object selectMany extends ProductArgs {
    def applyProduct[U <: HList, Out0 <: HList, Out](columns: U)(
        implicit i0: ColumnTypes.Aux[T, U, Out0],
        i1: ToTraversable.Aux[U, List, UntypedExpression[T]],
        i2: Tupler.Aux[Out0, Out],
        i3: TypedEncoder[Out]): TypedDataset[Out] = {
      val base = dataset
  ...
  object sortWithinPartitionsMany extends ProductArgs {
    def applyProduct[U <: HList, O <: HList](columns: U)(
        implicit i0: Mapper.Aux[SortedTypedColumn.defaultAscendingPoly.type, U, O],
        i1: ToTraversable.Aux[O, List, SortedTypedColumn[T, _]]): TypedDataset[T] = {
      val sorted = dataset
        .toDF()
  ...
  def dropTupled[Out, TRep <: HList, Removed <: HList, ValuesFromRemoved <: HList, V](
      ...
      implicit i0: LabelledGeneric.Aux[T, TRep],
      i1: Remover.Aux[TRep, column.T, (V, Removed)],
      i2: Values.Aux[Removed, ValuesFromRemoved],
      i3: Tupler.Aux[ValuesFromRemoved, Out],
      i4: TypedEncoder[Out]): TypedDataset[Out] = {
    val dropped = dataset.toDF().drop(column.value.name).as[Out](TypedExpressionEncoder[Out])
  ...
  )(implicit i0: TypedColumn.Exists[T, column.T, A]): TypedDataset[T] = {
    val updated = dataset
      .toDF()
  ...
  class WithColumnApply[U] {
    def apply[
        ...
        NewFields <: HList,
        NewKeys <: HList,
        NewKey <: Symbol](ca: TypedColumn[T, A])(
        implicit i0: TypedEncoder[U],
        i1: TypedEncoder[A],
        ...
        iA: ToTraversable.Aux[UKeys, Seq, Symbol]): TypedDataset[U] = {
      val newColumnName =
        i7.head(i6()).name
      ...
      val selected = dfWithNewColumn.select(newColumns: _*).as[U](TypedExpressionEncoder[U])
  ...
  def explode[A, TRep <: HList, V[_], OutMod <: HList, OutModValues <: HList, Out](
      column: Witness.Lt[Symbol])(
      implicit i0: TypedColumn.Exists[T, column.T, V[A]],
      i1: TypedEncoder[A],
      ...
      i7: TypedEncoder[Out]): TypedDataset[Out] = {
    val df = dataset.toDF()
...
object TypedDataset {
  def create[A](data: Seq[A])(
      ...
      sqlContext: SparkSession): TypedDataset[A] = {
  ...
  def create[A: TypedEncoder](dataset: Dataset[A]): TypedDataset[A] = createUnsafe(
    dataset.toDF())
  ...
  def createUnsafe[A: TypedEncoder](df: DataFrame): TypedDataset[A] = {
    ...
    val shouldReshape =
      output.zip(targetColNames).exists { case (expr, colName) => expr.name != colName }
