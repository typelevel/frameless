...
class PivotTest extends TypedDatasetSuite {
  ...
  test("X4[Boolean, String, Int, Boolean] pivot on String") {
    def prop(data: Vector[X4[String, String, Int, Boolean]]): Prop = {
      ...
      val frameless = d.groupBy(d(Symbol("a"))).
        pivot(d(Symbol("b"))).on("a", "b", "c").
        agg(sum(d(Symbol("c"))), first(d(Symbol("d")))).collect().run().toVector
  ...
  test("Pivot on Boolean") {
    ...
    d.groupByMany(d(Symbol("a"))).
      pivot(d(Symbol("c"))).on(true, false).
