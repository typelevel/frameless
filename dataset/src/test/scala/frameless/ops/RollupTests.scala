...
class RollupTests extends TypedDatasetSuite {
  ...
  test("rollup('a).agg(count())") {
    def prop[A: TypedEncoder: Ordering, Out: TypedEncoder: Numeric](data: List[X1[A]])(
        implicit summable: CatalystSummable[A, Out]): Prop = {
  ...
  test("rollup('a, 'b).agg(count())") {
    def prop[A: TypedEncoder: Ordering, B: TypedEncoder, Out: TypedEncoder: Numeric](
      ...
      val expected = dataset
        ...
        .collect()
        .toVector
        .map(row => (Option(row.getAs[A](0)), Option(row.getAs[B](1)), row.getAs[Long](2)))
        .sortBy(_._3)
  ...
  test(
    ...
    def prop[
        ...
        implicit summableB: CatalystSummable[B, OutB],
      ...
      val framelessSumBC =
        dataset.rollup(A).agg(sum(B), sum(C)).collect().run().toVector.sortBy(_._1)

      val sparkSumBC = dataset
        .dataset
        .rollup("a")
        .sum("b", "c")
        .collect()
      ...
      val framelessSumBCB =
        dataset.rollup(A).agg(sum(B), sum(C), sum(B)).collect().run().toVector.sortBy(_._1)

      val sparkSumBCB = dataset
        .dataset
        .rollup("a")
        .sum("b", "c", "b")
        .collect()
        .toVector
        .map(row =>
          (Option(row.getAs[A](0)), row.getAs[OutB](1), row.getAs[OutC](2), row.getAs[OutB](3)))
        .sortBy(_._1)
  ...
  test("rollup('a, 'b).agg(sum('c), sum('d))") {
    def prop[
        ...
        implicit summableC: CatalystSummable[C, OutC],
      ...
      val framelessSumByAB =
        dataset.rollup(A, B).agg(sum(C), sum(D)).collect().run().toVector.sortBy(_._2)

      val sparkSumByAB = dataset
        .dataset
        .rollup("a", "b")
        .sum("c", "d")
        .collect()
  ...
  test(
    ...
    def prop[
      ...
      val framelessSumC = dataset.rollup(A, B).agg(sum(C)).collect().run().toVector.sortBy(_._2)

      val sparkSumC = dataset
        .dataset
        .rollup("a", "b")
        .sum("c")
        .collect()
      ...
      val framelessSumCC =
        dataset.rollup(A, B).agg(sum(C), sum(C)).collect().run().toVector.sortBy(_._2)

      val sparkSumCC = dataset
        .dataset
        .rollup("a", "b")
        .sum("c", "c")
        .collect()
      ...
      val framelessSumCCC =
        dataset.rollup(A, B).agg(sum(C), sum(C), sum(C)).collect().run().toVector.sortBy(_._2)

      val sparkSumCCC = dataset
        .dataset
        .rollup("a", "b")
        .sum("c", "c", "c")
        .collect()
  ...
  test("rollup('a).flatMapGroups(('a, toVector(('a, 'b))") {
    def prop[
      ...
      val dataGrouped =
        data.groupBy(_.a).toSeq.flatMap { case (a, xs) => xs.map(x => (a, x)) }.sorted
  ...
  test("rollupMany('a).agg(sum('b))") {
    def prop[A: TypedEncoder: Ordering, Out: TypedEncoder: Numeric](data: List[X1[A]])(
      ...
      val received =
        dataset.rollupMany(A).agg(count[X1[A]]()).collect().run().toVector.sortBy(_._2)
      val expected = dataset
        .dataset
        .rollup("a")
        .count()
