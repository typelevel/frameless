...
class CubeTests extends TypedDatasetSuite {
  ...
  test(
    ...
    def prop[
      ...
      val framelessSumBC =
        dataset.cube(A).agg(sum(B), sum(C)).collect().run().toVector.sortBy(_._1)
      ...
      val framelessSumBCB =
        dataset.cube(A).agg(sum(B), sum(C), sum(B)).collect().run().toVector.sortBy(_._1)
  ...
  test(
    ...
    def prop[
      ...
      val framelessSumC = dataset.cube(A, B).agg(sum(C)).collect().run().toVector.sortBy(_._2)
      ...
      val framelessSumCC =
        dataset.cube(A, B).agg(sum(C), sum(C)).collect().run().toVector.sortBy(_._2)
      ...
      val framelessSumCCC =
        dataset.cube(A, B).agg(sum(C), sum(C), sum(C)).collect().run().toVector.sortBy(_._2)
  ...
  test("cubeMany('a).agg(sum('b))") {
    def prop[A: TypedEncoder: Ordering, Out: TypedEncoder: Numeric](data: List[X1[A]])(
      ...
      val received =
        dataset.cubeMany(A).agg(count[X1[A]]()).collect().run().toVector.sortBy(_._2)
      val expected = dataset
        .dataset
        .cube("a")
        .count()
