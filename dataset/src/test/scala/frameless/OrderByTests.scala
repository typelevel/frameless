...
class OrderByTests extends TypedDatasetSuite with Matchers {
  def sortings[A: CatalystOrdered, T]
      : Seq[(TypedColumn[T, A] => SortedTypedColumn[T, A], Column => Column)] = Seq(
    (_.desc, _.desc),
    (_.asc, _.asc),
      ...
      sortings[A, X2[A, B]]
        ...
        .map {
          case ((typA, untypA), (typB, untypB)) =>
            val vanillaSpark = ds
              .dataset
              .sortWithinPartitions(untypA(ds.dataset.col("a")), untypB(ds.dataset.col("b")))
              .collect()
              .toVector
            vanillaSpark
              .?=(ds.sortWithinPartitions(typA(ds('a)), typB(ds('b))).collect().run().toVector)
              .&&(
                vanillaSpark ?= ds
      ...
      sortings[A, X3[A, B, A]]
        ...
        .map {
          case (((typA, untypA), (typB, untypB)), (typA2, untypA2)) =>
            ...
            vanillaSpark
              .?=(
                ds.orderBy(typA(ds('a)), typB(ds('b)), typA2(ds('c))).collect().run().toVector)
              .&&(
                vanillaSpark ?= ds
      ...
      sortings[A, X3[A, B, A]]
        ...
        .map {
          case (((typA, untypA), (typB, untypB)), (typA2, untypA2)) =>
            ...
            vanillaSpark
              .?=(
                ds.sortWithinPartitions(typA(ds('a)), typB(ds('b)), typA2(ds('c)))
                  .collect()
                  .run()
                  .toVector)
              .&&(
                vanillaSpark ?= ds
                  .sortWithinPartitionsMany(typA(ds('a)), typB(ds('b)), typA2(ds('c)))
                  .collect()
                  .run()
                  .toVector
              )
