...
class GroupByTests extends TypedDatasetSuite {
  test("groupByMany('a).agg(sum('b))") {
    def prop[
        ...
        implicit summable: CatalystSummable[B, Out],
        widen: B => Out
    ): Prop = {
  ...
  test("agg(sum('a), sum('b))") {
    def prop[
        ...
        implicit as: CatalystSummable[A, A],
  ...
  test(
    ...
    def prop[
      ...
      val framelessSumBC =
        dataset.groupBy(A).agg(sum(B), sum(C)).collect().run.toVector.sortBy(_._1)

      val scalaSumBC = data
        .groupBy(_.a)
        .mapValues { xs => (xs.map(_.b).map(widenb).sum, xs.map(_.c).map(widenc).sum) }
        .toVector
        .map { case (a, (b, c)) => (a, b, c) }
        .sortBy(_._1)

      val framelessSumBCB =
        dataset.groupBy(A).agg(sum(B), sum(C), sum(B)).collect().run.toVector.sortBy(_._1)
  ...
  test(
    ...
    def prop[
      ...
      val framelessSumC =
        dataset.groupBy(A, B).agg(sum(C)).collect().run.toVector.sortBy(x => (x._1, x._2))
      ...
      val scalaSumCCCCC = data
        ...
        .toVector
        .map { case ((a, b), (c1, c2, c3, c4, c5)) => (a, b, c1, c2, c3, c4, c5) }
        .sortBy(x => (x._1, x._2))
  ...
  test("groupBy('a, 'b).agg(sum('c), sum('d))") {
    def prop[
      ...
      val sumByAB = data
        .groupBy(x => (x.a, x.b))
        .mapValues { xs => (xs.map(_.c).map(widenc).sum, xs.map(_.d).map(widend).sum) }
        .toVector
