...
```tut:book
val cities: TypedDataset[String] = aptTypedDs.select(aptTypedDs(_.city))
```

This is completely type-safe, for instance suppose we misspell `city` as `citi`:

```tut:book:fail
aptTypedDs.select(aptTypedDs(_.citi))
```
...
```tut:book
aptTypedDs.select(aptTypedDs(_.surface) * 10, aptTypedDs(_.surface) + 2).show().run()
```
...
```tut:book:fail
val priceBySurfaceUnit = aptTypedDs.select(aptTypedDs(_.price) / aptTypedDs(_.surface))
```
...
```tut:book
val priceBySurfaceUnit = aptTypedDs.select(aptTypedDs(_.price) / aptTypedDs(_.surface).cast[Double])
...
val updated = aptTypedDs.select(aptTypedDs(_.city), aptTypedDs(_.surface) + 2).as[UpdatedSurface]
updated.show(2).run()
```
...
```tut:book:fail
aptTypedDs.select(aptTypedDs(_.city), aptTypedDs(_.city)).as[UpdatedSurface]
```
...
```tut:book
val priceByCity = aptTypedDs.groupBy(aptTypedDs(_.city)).agg(avg(aptTypedDs(_.price)))
priceByCity.collect().run()
```
Again if we try to aggregate a column that can't be aggregated, we get a compilation error
```tut:book:fail
aptTypedDs.groupBy(aptTypedDs(_.city)).agg(avg(aptTypedDs(_.city)))                                                         ^
```
...
```tut:book
val withCityInfo = aptTypedDs.join(citiInfoTypedDS, aptTypedDs(_.city), citiInfoTypedDS(_.name))
