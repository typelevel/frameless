...
class TypedKMeans[Inputs] private[ml] (
  ...
  val estimator: KMeans =
    km.setFeaturesCol(featuresCol).setPredictionCol(AppendTransformer.tempColumnName)

  def setK(value: Int): TypedKMeans[Inputs] = copy(km.setK(value))

  def setInitMode(value: KMeansInitMode): TypedKMeans[Inputs] = copy(
    km.setInitMode(value.sparkValue))
...
object TypedKMeans {
  ...
  def apply[Inputs](
      implicit inputsChecker: VectorInputsChecker[Inputs]): TypedKMeans[Inputs] = {
