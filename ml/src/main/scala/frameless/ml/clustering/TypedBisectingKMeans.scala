 ...
 * techniques" by Steinbach, Karypis, and Kumar, with modification to fit Spark. The algorithm
 * starts from a single cluster that contains all points. Iteratively it finds divisible
 * clusters on the bottom level and bisects each of them using k-means, until there are `k` leaf
 * clusters in total or no leaf clusters are divisible. The bisecting steps of clusters on the
 * same level are grouped together to increase parallelism. If bisecting all divisible clusters
 * on the bottom level would result more than `k` leaf clusters, larger clusters get higher
 * priority.
 *
 * @see
 *   <a href="http://glaros.dtc.umn.edu/gkhome/fetch/papers/docclusterKDDTMW00.pdf"> Steinbach,
 *   Karypis, and Kumar, A comparison of document clustering techniques, KDD Workshop on Text
 *   Mining, 2000.</a>
 */
