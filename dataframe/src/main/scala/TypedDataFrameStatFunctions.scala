...
final class TypedDataFrameStatFunctions[Schema <: Product] private[frameless]
  ...
  // This can't a TypedDataFrame since the output columns depend on the dataframe
  ...
  class FreqItemsCurried(support: Double) extends SingletonProductArgs {
    def applyProduct[Out <: Product, C <: HList, S <: HList]
      ...
      (implicit
        ...
        new TypedDataFrame(dfs.freqItems(f(columns), support))
  }
  ...
  def sampleBy[T, G <: HList, C]
    ...
    (implicit
      ...
      new TypedDataFrame(dfs.sampleBy(column.value.name, fractions, seed))
}
